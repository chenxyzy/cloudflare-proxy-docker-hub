...
export default {
  async fetch(request, env, ctx) {
    ...
    const data = await original_response_clone.clone().text(); // 或 response.json()
    //console.log('---new response data:---');
    //console.log(data);
    let original_text = original_response_clone.body;
    let response_headers = original_response.headers;
    //console.log('---new response headers:---');
    //console.log(new Headers(response_headers));
...
function httpHandler(req, pathname, baseHost) {
  ...
  let reqHdrNew = new Headers(reqHdrRaw);
